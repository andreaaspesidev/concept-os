...
fn elf_parse(elf_buffer: &Vec<u8>) -> Result<Elf32, Box<dyn Error>> {
    ...
    if elf_result.is_err() {
        return Err(Box::new(CbfError{msg: String::from("Cannot read ELF")}));
    }
    let elf = elf_result.unwrap();
    return Ok(elf);
}
fn elf_check(component_elf: &Elf32) -> Result<(), CbfError> {
    if component_elf.elf_header().endianness() != ElfEndian::LittleEndian {
        return Err(CbfError{msg: String::from("Wrong endianness: little endian expected")});
    }
    if component_elf.elf_header().machine() != ElfMachine::ARM {
        return Err(CbfError{msg: String::from("Wrong machine: ARM supported only")});
    }
    Ok(())
}
...
fn elf_read_section(component_elf: &Elf32, section_name: &[u8]) -> Result<ElfSection, CbfError> {
    let section_result = component_elf.lookup_section(section_name);
    if section_result.is_none() {
        return Err(CbfError{msg: format!("Cannot find {:?} section in ELF", section_name)});
    }
    let section = section_result.unwrap();
...
fn elf_read_bss(component_elf: &Elf32) -> Result<u32, CbfError> {
    let section_result = component_elf.lookup_section(b".bss");
    if section_result.is_none() {
        return Err(CbfError{msg: format!("Cannot find .bss section in ELF")});
    }
    let section = section_result.unwrap();
    return Ok(section.size() as u32);
}   
fn offset_entrypoint(component_elf: &Elf32) -> Result<u32, CbfError> {
...
mod test {
    ...
    fn read_example3() {
        ...
        // .text section
        assert_eq!(result.text_section.address, 0x08000000);
        assert_eq!(result.text_section.size, 0x00120);
    ...
    fn read_example4() {
        ...
        // .text section
        assert_eq!(result.text_section.address, 0x08000000);
        assert_eq!(result.text_section.size, 0x000bc);
