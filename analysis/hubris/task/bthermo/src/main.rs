...
use bthermo_api::{
    GetOutputStatusResponse, GetProgramsResponse, Operation, OutputType,
    Program, ReadRTCResponse, ReadTemperatureResponse, RemoveProgramRequest,
    RepeatType, SetProgramRequest, SetProgramResponse, SetRTCRequest,
    ThermoError, TimeStructure, MAX_PROGRAMS,
};
use rcc_api::RCC;
...
fn process_requests(
    ...
    loop {
        let result = sys_recv(&mut request_buffer, TIMER_IRQ_MASK, None);
        if result.is_ok() {
            // Parse message
                ...
                match operation.unwrap() {
                    ...
                    Operation::SetRTC => {
                        ...
                        if result.is_ok() {
                            ...
                            sys_reply(
                                rm.sender,
                                ThermoError::RTCNotConnected.into(),
                                &[],
                            );
                        }
                    }
                    Operation::ReadTemperature => {
                        let response = ReadTemperatureResponse {
                            ...
                            operation_value: history
                                .perform_operation(state_manager),
                        };
                        sys_reply(rm.sender, 0, response.as_bytes());
                    }
                    Operation::GetOutputs => {
                        let response = GetOutputStatusResponse {
                            outputs_active: output_controller
                                .get_status()
                                .map(|s| s as u8),
                        };
                        sys_reply(rm.sender, 0, response.as_bytes());
                    ...
                    Operation::GetPrograms => {
                        let programs =
                            &state_manager.get_program_state().programs;
                        ...
                        for (i, p) in programs.iter().enumerate() {
                            ...
                            response_programs[i].temperature_setpoint =
                                p.temperature_setpoint;
                    ...
                    Operation::SetProgram => {
                        ...
                        match result {
                            Ok(p_id) => {
                                let response =
                                    SetProgramResponse { program_id: p_id };
                                sys_reply(rm.sender, 0, response.as_bytes());
                            }
                            Err(_) => {
                                sys_reply(
                                    rm.sender,
                                    ThermoError::NoMoreSpace.into(),
                                    &[],
                                );
                            }
                        }
                    ...
                    Operation::RemoveProgram => {
                        let msg =
                            LayoutVerified::<_, RemoveProgramRequest>::new(
                                &request_buffer[..rm.message_len],
                            )
                            .unwrap_lite()
                            .into_ref();
                        ...
                        let result = program_manager
                            .remove_program(program_id, state_manager);
                        if result.is_ok() {
                            sys_reply(rm.sender, 0, &[]);
                        } else {
                            sys_reply(
                                rm.sender,
                                ThermoError::MissingProgram.into(),
                                &[],
                            );
                        }
                    }
