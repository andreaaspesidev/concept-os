    ...
    > ListBuddyImpl<START_ADDR, END_ADDR, BLOCK_SIZE, NUM_BLOCKS, NUM_SLOTS>
{
    ...
    pub fn new(skip_initialization: bool) -> Self {
...
impl<
        ...
        const NUM_BLOCKS: usize,
        const NUM_SLOTS: usize,
    > BuddyAllocator for ListBuddyImpl<START_ADDR, END_ADDR, BLOCK_SIZE, NUM_BLOCKS, NUM_SLOTS>
{
    ...
    fn add_free_block(&mut self, block_num: usize) -> Option<()> {
        self.add_free_block(block_num as u8, NUM_SLOTS - 1)
    }

    fn is_all_available(&self) -> bool {
        self.free_lists[0].contains(&0)
    }
}
